package www.pdx.life.ykmtools.viewUtils.advice;
...
import www.pdx.life.ykmtools.tools.BitmapUtils;
...
public class FlyBanner extends RelativeLayout {
    ...
    private FlyPageAdapter adapter = null;
    ...
    public void setImages(List<Integer> images) {
        mViewPager.setOffscreenPageLimit(images.size() + 1);          //添加缓存，有效提升性能
        //加载本地图片
        mIsImageUrl = false;
        this.mImages = images;
        if (images.size() <= 1)
    ...
    public void setImagesUrl(List<String> urls) {
        mViewPager.setOffscreenPageLimit(urls.size() + 1);          //添加缓存，有效提升性能
        //加载网络图片
        mIsImageUrl = true;
    ...
    private class FlyPageAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(ViewGroup container, final int position) {
            ...
            if (mIsImageUrl) {
                // TODO: 2016/7/30 加载网络图片
                Picasso.with(getContext())
                        .load(mImageUrls.get(toRealPosition(position)))
                        .placeholder(R.drawable.pdx1)
                        .error(R.drawable.pdx2)
                        .into(imageView);
            } else {
                Bitmap bitmap = BitmapUtils.decodeSampledBitmapFromResource(
                        getResources(), mImages.get(toRealPosition(position)), 300, 300);
